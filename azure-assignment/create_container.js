...
const create_container = (input) => {
  const container_url =
    ...
    input +
    "/blobServices/default/containers/data?api-version=2021-09-01";
  request(
    {
      method: "PUT",
      headers: {
    ...
    (error, response, body) => {
      ...
      if (error) {
        ...
        console.log("Error! Please check storage account name or auth token.");
      } else {
        console.log(body.name + " container created!");
      }
    }
  );
};
