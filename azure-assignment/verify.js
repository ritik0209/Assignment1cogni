...
const verify = (input) => {
  const verify_url =
    "https://management.azure.com/subscriptions/c5783c61-3b68-4f04-b3c4-212b19ecc864/providers/Microsoft.Storage/checkNameAvailability?api-version=2021-09-01";
  ...
  request.post(
    ...
    (error, response, body) => {
      if (error) {
        ...
        if (body.nameAvailable === false) {
          console.log("Storage account name not valid.");
        } else {
          console.log(body);
        }
      }
